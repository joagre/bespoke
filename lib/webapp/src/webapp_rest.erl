...
http_get(Socket, Request, Url, Tokens, Body, _State, v1) ->
    case handle_captive_portal(Socket, Request, Tokens) of
        yes ->
            ok;
        no ->
            Headers = Request#http_request.headers,
            case Tokens of
                %% Captive portal acks are generated by /loader.html
                ["api", "captive_portal_ack"] ->
                    ...
                    PortalCacheEntry = #portal_cache_entry{ip_address = IpAddress,
...
handle_captive_portal(Socket, Request, [Page]) ->
    case is_captive_portal(Request, Page) of
        true ->
            {ok, {IpAddress, _Port}} = rester_socket:peername(Socket),
            case ets:lookup(?PORTAL_CACHE, IpAddress) of
                [] ->
                    ?log_info("Captive portal: Replies 200 OK + /loader.html body"),
                    send_loader_page(Socket, Request);
                [#portal_cache_entry{timestamp = Timestamp}] ->
                    case timestamp() - Timestamp > ?PORTAL_TIMEOUT of
                        true ->
                            ?log_info(
                               "Captive portal (timeout): Replies 200 OK + /loader.html body"),
                            send_loader_page(Socket, Request);
                        false when Page == "hotspot-detect.html" orelse
                                   Page == "success.html" ->
                            ?log_info("Captive poeral: Replies 200 OK (Apple mode)\n"),
                            rester_http_server:response_r(
                              Socket, Request, 200, "OK",
                              "<HTML><HEAD></HEAD><BODY>Success</BODY></HTML>",
                              [{content_type, "text/html"}|no_cache_headers()]);
                        false when Page == "connecttest.txt" ->
                            ?log_info("Captive portal: Replies 200 OK (Windows mode)\n"),
                            rester_http_server:response_r(
                              Socket, Request, 200, "OK", "Microsoft Connect Test",
                              [{content_type, "text/plain"},
                               no_cache_headers()]);
                        false when Page == "ncsi.txt" ->
                            ?log_info("Captive portal: Replies 200 OK (Windows mode)\n"),
                            rester_http_server:response_r(
                              Socket, Request, 200, "OK", "Microsoft NCSI",
                              [{content_type, "text/plain"},
                               no_cache_headers()]);
                        false when Page == "success.txt" ->
                            ?log_info("Captive portal: Replies 200 OK (Firefox mode)\n"),
                            rester_http_server:response_r(
                              Socket, Request, 200, "OK", "success",
                              [{content_type, "text/plain"},
                               no_cache_headers()]);
                        false when Page == "canonical.html" ->
                            ?log_info("Captive portal: Replies 200 OK (Ubuntu mode)\n"),
                            rester_http_server:response_r(
                              Socket, Request, 200, "OK",
                              "<HTML><HEAD></HEAD><BODY>Success</BODY></HTML>",
                              [{content_type, "text/html"},
                               no_cache_headers()]);
                        _ ->
                            ?log_info("Returning 204 No Content (Generic mode)\n"),
                            rester_http_server:response_r( Socket, Request, 204, "OK", "",
                                                           no_cache_headers())
                    end
            end,
...
handle_captive_portal(_Socket, _Request, _Tokens) ->
    no.

is_captive_portal(Request, Page) ->
    Headers = Request#http_request.headers,
    case Page of
        %% iOS
        ...
        "success.html" when Headers#http_chdr.host == "detectportal.firefox.com" -> true;
        "success.txt" when Headers#http_chdr.host == "detectportal.firefox.com" -> true;
        %% Android
        "generate_204" -> true;
        "gen_204" -> true;
        %% Ubuntu
        ...
        _ when Headers#http_chdr.host == "connectivity-check.ubuntu.com." orelse
                 Headers#http_chdr.host == "connectivity-check.ubuntu.com" -> true;
        _ ->
            false
    end.
