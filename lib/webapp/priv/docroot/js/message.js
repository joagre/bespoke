...
const Message = (function () {
  ...
  async function _loadData() {
    ...
    try {
      ...
      const readMessageAndBodyResult = await MessageLib.readMessageAndBody(_topMessageId, _userId);
  ...
  async function _populatePage() {
    ...
    const attachments = await MessageLib.generateAttachments(_userId, _topMessage);
    if (attachments != "") {
  ...
  async function _createReplyTemplate(topMessage, message, replyMessages) {
    ...
    if (message.parentMessageId == _topMessageId) {
      ...
      parentReplyMessage = replyMessages.find(
    ...
    const replyQuote = html`
      <div class="uk-text-meta">
        <div class="quote" onclick=${event => _toggleQuote(event)}>
    ...
    const attachments = await MessageLib.generateAttachments(_userId, message);
  ...
  function _toggleQuote(event) {
    ...
    if (replyQuoteElement.hidden) {
      ...
      let replyBodyElement;
      if (parentMessageId == _topMessageId) {
        replyBodyElement = _topBodyElement;
      } else {
        replyBodyElement = document.getElementById(`reply-body-${parentMessageId}`);
      }
      replyQuoteBodyElement.innerHTML = replyBodyElement.innerHTML;

      // Show reply quote
      replyQuoteElement.hidden = false;
      replyQuoteActionElement.setAttribute("uk-icon", "chevron-up");
    } else {
      // Hide reply quote
