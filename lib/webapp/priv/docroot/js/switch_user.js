  ...
  async function _updateServer() {
    try {
      ...
      if (_formPasswordElement.value.length > 0) {
        const generateChallengeResponse = await fetch("/api/generate_challenge", {
          method: "POST",
          headers: {
        ...
        if (!generateChallengeResponse.ok) {
          if (generateChallengeResponse.status === 401) {
            Bespoke.navigateTo("loader.html");
            return;
          } else if (generateChallengeResponse.status === 403) {
        ...
        const generateChallengeResult = await generateChallengeResponse.json();
        const clientResponseResult =
              await _generateClientResponse(
                _formPasswordElement.value,
                Bespoke.decodeBase64(generateChallengeResult.passwordSalt),
                Bespoke.decodeBase64(generateChallengeResult.challenge));
        encodedPasswordSalt = generateChallengeResult.passwordSalt;
      ...
      // Set cookies
      Bespoke.setCookieValue("bbsName", switchUserResult.bbsName);
      ...
      if (switchUserResult.isNew) {
        await MessageLib.createIntroductionMessage(switchUserResult.userId,
                                                   switchUserResult.username);
      }
