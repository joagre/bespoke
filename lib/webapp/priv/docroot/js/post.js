...
import topPosts from "/js/top_posts.js";
...
class Post {
  ...
  _load() {
    ...
    this._userId = bespoke.getCookieValue("userId");
  ...
  async _loadData() {
    ...
    try {
      ...
      bespoke.markAsRead(this._parentPost.id);
      ...
      const lookupRecursivePostsResponse =
            await fetch("/api/lookup_recursive_posts", {
              method: "POST",
              headers: {
                "Content-Type": "application/json"
              },
  ...
  _populatePage() {
    ...
    const attachments = topPosts.generateAttachments(this._parentPost);
    if (attachments != "") {
      render(document.getElementById("parent-attachments"),
             html`${attachments}`);
    ...
    if (this._parentPost.likers.includes(this._userId)) {
  ...
  _createReplyTemplate(parentPost, post, replyPosts) {
    ...
    if (post.parentPostId != null &&
      ...
      replyQuote = html`
        ...
        <div class="uk-text-meta">
          <div class="quote" onclick=${(event) => this.toggleQuote(event)}>
    ...
    // Like
    let likeAttr;
    if (post.likers.includes(this._userId)) {
