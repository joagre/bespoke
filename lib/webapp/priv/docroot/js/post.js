...
class Post {
  ...
  _createReplyTemplate(parentPost, post, replyPosts) {
    ...
    if (post.likers.includes(bespoke.getCookieValue("userId"))) {
      likeAttr = "liked uk-icon-button";
    } else {
      likeAttr = "uk-icon-button";
    }
    // Replies
    let replies = "";
    if (post.replyCount > 0) {
      replies = html`•
        <button onclick=${(event) => bespoke.gotoPage(event, "/post.html", post.id)} class="uk-icon-button" uk-icon="comment"></button>
        ...
        <div class="uk-flex uk-flex-between uk-flex-middle">
          ...
          <div class="uk-text-meta meta-data">
            ...
            <button onclick=${(event) => this.toggleLike(event)}
                    class="uk-icon-button" class="${likeAttr}" uk-icon="icon: heart"></button>
