...
const Post = (function () {
  ...
  async function _loadData() {
    ...
    try {
      ...
      if (!(_userId == 0 || _parentPost.authorId == _userId)) {
  ...
  async function _populatePage() {
    ...
    await StickyHeader.adjust(_stickyHeaderElement, _mainContentElement);
    ...
    const keyDown = (key, section) => {
      if (key == "Enter") {
        ...
        AddReplyPost.gotoAddReplyPage(null, postId, true)
        ...
        Bespoke.gotoPage(event, "post.html", -1);
      }
    };
    ...
    if (post.parentPostId != null &&
      ...
      replyQuote = html`
        <div class="uk-text-meta">
          <div class="quote" onclick=${event => toggleQuote(event)} title="Quote">
    ...
    if (post.replyCount > 0) {
      replies = html`•
        <button onclick=${event => Bespoke.gotoPage(event, "/post.html", post.id)}
                class="toolbar-button" uk-icon="comment" title="Replies"></button>
        ${post.replyCount}`;
    }

    // Delete button
    let deleteButton = "";
    if (_userId === 0 || post.authorId === _userId) {
      deleteButton = html`
      ...
      <div class="post-section"
        ...
        <div class="uk-flex uk-flex-between uk-flex-middle">
          <div class="uk-text-meta post-meta-data meta-data">
            <span class="unread" uk-icon="bolt" title="Unread"></span>
            ${author} •
            ${age} •
          ...
          <div>
            ...
            <button onclick=${event => AddReplyPost.gotoAddReplyPage(event, post.id, true)}
                    class="toolbar-button" uk-icon="reply" title="Reply"></button>
          </div>
        </div>
