...
const Index = (function () {
  ...
  function _load() {
    ...
    // Intro
    const logotypeElement = document.getElementById("logotype");
    window.addEventListener("load", () => {
      // Wait for the initial fade-in plus a short pause
      setTimeout(() => {
        // Clear old animation so we can restart it
        UIkit.util.removeClass(logotypeElement, "uk-animation-fade");
        // Force reflow, i.e. browser forgets the old animation state
        void logotypeElement.offsetWidth; // Magic reflow line
        // Set the new (long) duration *before* we start the next run
        logotypeElement.style.animationDuration = "0.5s";
        // Start a new fade with reverse direction
        UIkit.util.addClass(logotypeElement, "uk-animation-fade uk-animation-reverse");
        // Remove element when the fade-out ends
        logotypeElement.addEventListener("animationend", () => {
          logotypeElement.style.display = "none";
        }, { once: true });
      }, 250);
    });
