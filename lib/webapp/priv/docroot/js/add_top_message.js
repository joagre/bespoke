...
const AddTopMessage = (function () {
  ...
  function _load() {
    ...
    if (!previousPage.endsWith("add_attachments.html")) {
      ...
      Bespoke.setLocalItem("selectedRecipients", []);
  ...
  function _searchRecipients(event) {
    ...
    const payload = {
      ignoreRecipients: _selectedRecipients,
      query: query
    };
    fetch("/api/search_recipients", {
      ...
      .then(recipients => {
        ...
        _populateRecipients(query, recipients);
      })
      .catch(error => {
  ...
  function _populateRecipients(query, recipients) {
    // Find matches
    ...
    if (matches.length > 0) {
      ...
      for (let i = 0; i < matches.length; i++) {
        if (!_selectedRecipients.includes(matches[i])) {
  ...
  function _checkFormCompletion() {
    _addButton.disabled = (_selectedRecipients.length === 0 ||
  ...
  function _updatePage() {
    ...
    // Update form
    _selectedRecipients = Bespoke.getLocalItem("selectedRecipients", []);
  ...
  async function _updateServer() {
    ...
    for (const recipient of _selectedRecipients) {
      try {
        _showProgressBar();
        // Upload body
        ...
        console.log(`Uploading body for ${recipient}...`);
        const uploadController = Bespoke.uploadData(body, "body", onProgress);
...
/*
  ...
  function gotoAddAttachments(event) {
    Bespoke.setLocalItem("selectedRecipients", _selectedRecipients);
  ...
  function _addSelectedRecipient(match) {
    _selectedRecipients.push(match);
    _refreshSelectedRecipients();
  }

  function _refreshSelectedRecipients() {
    _checkFormCompletion();
    _formSelectedRecipients.hidden = _selectedRecipients.length === 0;
    const recipients = [];
    for (const selectedRecipient of _selectedRecipients) {
      const recipientSpan =
            html`<span class="uk-label no-uppercase uk-margin-small-right">${selectedRecipient}<a class="uk-margin-small-left uk-link-reset" onclick=${(_event) => {
                    _selectedRecipients =
                      _selectedRecipients.filter(recipient => recipient !== selectedRecipient);
                    _refreshSelectedRecipients();
                  }}>&times;</a></span>`;
