...
const AddTopMessage = (function () {
  ...
  function _searchUsernames(event) {
    ...
    const queryString = event.currentTarget.value.trim().toLocaleLowerCase();
    ...
    const matches = recipients
          ...
          .filter(recipientObj => recipientObj.lower.includes(queryString.toLocaleLowerCase()))
    ...
    if (matches.length > 0) {
      ...
      for (let i = 0; i < matches.length; i++) {
        if (!_selectedRecipients.has(matches[i])) {
          const substrings = matches[i].split(new RegExp(`(${queryString})`, 'gi'));
          for (let i = 0; i < substrings.length; i++) {
            if (substrings[i].toLocaleLowerCase() === queryString) {
  ...
  function _refreshSelectedRecipients() {
    ...
    for (const selectedRecipient of _selectedRecipients) {
      const recipientSpan =
            html`<span class="uk-label no-uppercase uk-margin-small-right">${selectedRecipient}<a class="uk-margin-small-left uk-link-reset" onclick=${(event) => {
