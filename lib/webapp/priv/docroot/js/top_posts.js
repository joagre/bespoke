        ...
        if (attachment.contentType.startsWith("image/")) {
          item = html`<div class="attachment-item long-click" uk-lightbox>
            <a href="${absPath}" data-caption="${attachment.filename}">
              <img class="attachment-content" src="${absPath}" alt="">
            </a>
          </div>`;
        } else if (attachment.contentType.startsWith("video/")) {
          item = html`<div class="attachment-item long-click" uk-lightbox>
            <a href="${absPath}" data-caption="${attachment.filename}">
              <video class="attachment-content" controls autoplay loop muted playsinline>
                <source src="${absPath}" type="${attachment.contentType}">
          ...
          item = html`<div class="attachment-item long-click">
            <a href="${absPath}" data-caption="${attachment.filename}">
              <img class="attachment-content" src="/images/1x1.png" alt="">
    ...
    return html`
      <div class="top-post">
        <div onclick=${(event) => bespoke.gotoPage(event, "post.html", post.id)}
             class="uk-text-bold post-title">${post.title}</div>
        ${attachments}
