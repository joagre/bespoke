...
const MessageLib = (function () {
  ...
  async function createSimpleMessageAndBlob(title, body, userId, username) {
    await MessageLib.createMessageAndBlobs(title, body, null, null, [{userId, username}],
                                           [username], null);
  }
  ...
  async function createMessageAndBlobs(title, body, parentMessageId, topMessageId, recipients,
    ...
    for (const selectedRecipientUsername of selectedRecipientUsernames) {
      try {
        ...
        _addUploadController(uploadController, uploadControllers);
        ...
        _removeUploadController(uploadController, uploadControllers);
      } catch (error) {
    ...
    for (const file of files) {
      ...
      for (const selectedRecipientUsername of selectedRecipientUsernames) {
        try {
          ...
          _addUploadController(metadataUploadController, uploadControllers);
          const metadataUploadedFile = await Bespoke.waitForUpload(metadataUploadController);
          _removeUploadController(metadataUploadController, uploadControllers);
          ...
          _addUploadController(attachmentUploadController, uploadControllers);
          const attachmentUploadedFile = await Bespoke.waitForUpload(attachmentUploadController);
          _removeUploadController(attachmentUploadController, uploadControllers);
  ...
  function _addUploadController(uploadController, uploadControllers) {
  ...
  function _removeUploadController(uploadController, uploadControllers) {
    if (uploadControllers != null) {
  ...
  function refreshSelectedRecipients(checkFormCompletion, refreshSelectedRecipientsNow,
    ...
    console.log("Selected recipients:", selectedRecipientUsernames);



    formSelectedRecipientsElement.hidden = selectedRecipientUsernames.length < 2;
    const recipients = [];
    for (const selectedRecipientUsername of selectedRecipientUsernames) {
  ...
  async function createIntroductionMessage(userId, username) {
    console.log("Creating introduction message...");
    const title = "Welcome to Bespoke BBS!";
    const body = marked.parse("You're now connected to a local B3S appliance—a private, offline space for discussion, file sharing, and secure messaging. Nothing leaves this network unless you take it with you.\n\n\
Try posting your first topic in the forum or send an encrypted direct message to someone. Everything stays local and secure.\n\n\
This is your space. Make it count.\n\n\
<img src=\"/images/the-team.gif\" alt=\"The Bespoke team\" uk-tooltip=\"The Bespoke team\"> - The Bespoke Team\n");
    await MessageLib.createSimpleMessageAndBlob(title, body, userId, username);
  }
