  ...
  _addEventListeners() {
    ...
    this._attachments.addEventListener("itemshown", (e) => {
      this._itemFilename.firstChild.innerText =
        `${this._uploadedFiles[e.srcElement.attributes["data-index"].value].filename}`;
      this._itemFilename.hidden = false;
      this._itemCounter.firstChild.innerText =
        `${parseInt(e.srcElement.attributes["data-index"].value) + 1} / ${this._uploadedFiles.length}`;
