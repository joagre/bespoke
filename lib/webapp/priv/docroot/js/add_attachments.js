...
class AddAttachments {
  ...
  _load() {
    ...
    this._itemFilename = document.getElementById("item-filename");
    this._itemCounter = document.getElementById("item-counter");
  ...
  removeAttachment(event) {
    ...
    this.setUploadedFiles(this._uploadedFiles);
    this._renderSlideshow();
  }
  ...
  adornFileInput() {
    ...
    // Helper: Check if input type="file" is supported
    const isFileInputSupported = () => {
      const input = document.createElement("input");
      input.type = "file";
      return input.type === "file"; // Returns true if supported
    };

    // Helper: Check if the user is on Android
    const isAndroid = /Android/i.test(navigator.userAgent);

    // If input type="file" is unsupported and on Android, modify behavior
    if (fileInput && !isFileInputSupported() && isAndroid) {
      // Add a click event to redirect via intent
      fileInput.addEventListener("click", (event) => {
        event.preventDefault(); // Prevent the default file input behavior
        const intentUrl = `intent://${window.location.hostname}/add_attachments.html#Intent;scheme=http;end`;
        console.log("File input not supported. Redirecting via intent:", intentUrl);
        window.location.href = intentUrl;
      });

      // Optionally show a warning message below the file input
      const warningMessage = document.createElement("p");
      warningMessage.className = "uk-text-warning";
      warningMessage.textContent =
        "File uploads are not supported in this browser. Redirecting to the system browser.";
      fileInput.parentNode.insertBefore(warningMessage, fileInput.nextSibling);
    }
  }
}
