...
owIDAQAB`;
  ...
  async function _autoLogin() {
    ...
    try {
      ...
      // Create initial message in inbox
      if (result.isNew) {
        console.log("Creating initial message in inbox");
        const title = "Welcome to Bespoke!";
        const body = "This is your first message. From you to you. :-)";
        await MessageLib.createSimpleMessageAndBlob(title, body, result.userId, result.username);
      }
