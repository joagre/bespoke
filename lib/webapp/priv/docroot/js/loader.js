  ...
  function _initServiceWorker() {
    ...
    navigator.serviceWorker.register("/sw.js").then(async (registration) => {
      try {
        ...
        // 2) Fetch /signatures/sw.js.sig (as ArrayBuffer)
        const sigResponse = await fetch("/signatures/sw.js.sig", { cache: "no-store" });
        if (!sigResponse.ok) {
          throw new Error(`Failed to fetch /signatures/sw.js.sig. Status: ${sigResponse.status}`);
        }
        ...
        if (!isValid) {
          ...
          console.log("/sw.js signature verified; continuing");
