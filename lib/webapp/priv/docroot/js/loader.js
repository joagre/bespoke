...
owIDAQAB`;
  ...
  function init() {
    if (window.location.hostname !== "localhost" &&
        window.location.hostname !== "foo.b3s.zone" &&
        !_isIPv4Address(window.location.hostname)) {
      const targetUrl = `https://foo.b3s.zone${window.location.pathname}`;
  ...
  function _initServiceWorker() {
    ...
    if (!("serviceWorker" in navigator)) {
      ...
      localStorage.setItem("insecureWarningMessage", "Service Worker not supported");
      return;
    }
