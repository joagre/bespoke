...
const TopMessages = (function () {
  ...
  async function _renderMessages(topMessages) {
    ...
    for (const topMessage of topMessages) {
      const template = await _createTopMessageTemplate(topMessage);
      _messagesElement.appendChild(html.node`${template}`);
    }
  ...
  async function _createTopMessageTemplate(topMessage) {
    ...
    if (!topMessage.isRead) {
      ...
      if (topMessage.readCount > 0) {
        ...
        replies = html`• <span uk-icon="commenting"></span>
                         ${topMessage.readCount} (${replyCount})`;
      }
    }
