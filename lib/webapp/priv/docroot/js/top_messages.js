...
const TopMessages = (function () {
  ...
  async function _renderMessages(topMessages) {
    ...
    const topMessageTemplates = await Promise.all(
      topMessages.map(async topMessage => {
        return await _createTopMessageTemplate(topMessage);
      }));
    render(_messagesElement, html`${topMessageTemplates}`);
  ...
  async function _createTopMessageTemplate(topMessage) {
    ...
    if (!topMessage.isRead ||
        !(topMessage.readCount ==  0 || replyCount == topMessage.readCount)) {
      unread = html`<span class="unread" uk-icon="bolt" uk-tooltip="Unread"></span>`;
    }

    // Author
    const author = html`<span uk-tooltip="Author">${topMessage.authorUsername}</span>`;

    // Age
    const age = html`
      <span uk-tooltip="Created">${Bespoke.formatSecondsSinceEpoch(topMessage.created)}</span>`;

    // Number of attachments
    let numberOfAttachments = "";
    if (topMessage.attachmentIds.length > 0) {
      numberOfAttachments = html`
        • <span uk-tooltip="Attachments">
            <span uk-icon="download"></span>${topMessage.attachmentIds.length}
          </span>`;
    }
    ...
    if (replyCount == 0) {
      replies = "";
    } else if (topMessage.readCount == 0 || replyCount == topMessage.readCount) {
      replies = html`
        • <span uk-tooltip="Replies">
            <span uk-icon="comment"></span>
            ${replyCount}
          </span>`;
    } else {
      replies = html`
        • <span uk-tooltip="Unread replies">
