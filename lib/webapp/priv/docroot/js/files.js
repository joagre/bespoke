...
const Files = (function () {
  ...
  function _initFileUpload() {
    ...
    _fileUpload.addEventListener("change", async function (e) {
      try {
        ...
        const insertedFile = await response.text();
        console.log("insertedFile:", insertedFile);
        _updatePage();
        // Upload file
        function onProgress(_total, _loaded, percentComplete) {
          console.log(`Upload progress: ${percentComplete.toFixed(2)}%`);
        }
        const uploadedFile = await Bespoke.uploadFile(file, onProgress);
  ...
  function _createFileTemplate(insertedFile) {
    ...
    if (insertedFile.uploader === Bespoke.getCookieValue("username")) {
      deleteButton =
        html`<button onclick=${event => File.deleteFile(event)}
    ...
    if (insertedFile.isUploading) {
      return html`
        <div data-file-id="${insertedFile.id}"
          ...
          <div>
            <a class="uk-link-text uk-text-bold disabled-link">${insertedFile.filename}</a>
          ...
          <div class="uk-flex uk-flex-between uk-flex-middle">
            <div class="file-meta-data uk-text-meta">
              ...
              ${Bespoke.formatBytes(insertedFile.size)}/<span class="loaded-bytes">0B</span>
            </div>
            <div>
              ${deleteButton}
            </div>
      ...
      return html`
        <div data-file-id="${insertedFile.id}"
             data-filename="${insertedFile.filename}">
          <div>
            <a href="/file/${insertedFile.id}-${insertedFile.filename}"
