...
const Files = (function () {
  ...
  function _createFileTemplate(createdFile) {
    ...
    // Uploader
    const uploader = html`<span uk-tooltip="Uploader">${createdFile.uploaderUsername}</span>`;

    // Age
    const age = html`
      <span uk-tooltip="Created">${Bespoke.formatSecondsSinceEpoch(createdFile.created)}</span>`;
        ...
        <div data-file-id="${createdFile.id}" data-filename="${createdFile.filename}">
          <div>
            ...
            <button onclick=${event => _copyToClipboard(event, window.location.origin + hrefAttr)}
                    class="toolbar-button" uk-icon="icon: copy" uk-tooltip="Copy link"></button>
            <span class="uk-text-meta">(${createdFile.contentType})</span>
          </div>
          <div class="uk-flex uk-flex-between uk-flex-middle">
            <div class="file-meta-data uk-text-meta">
