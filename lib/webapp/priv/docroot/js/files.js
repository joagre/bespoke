...
const Files = (function () {
  ...
  function _createFileTemplate(createdFile) {
    // Truncated filename
    const truncatedFilename = Bespoke.truncateMiddle(createdFile.filename, 48);

    // Uploader
    const uploader = html`<span title="Uploader">${createdFile.uploaderUsername}</span>`;

    // Age
    const age = html`
      <span title="Created">${Bespoke.formatSecondsSinceEpoch(createdFile.created)}</span>`;

    // Delete button
    let deleteButton = "";
    if (_userId === 0 || createdFile.uploaderId === Bespoke.getCookieValue("userId")) {
      deleteButton =
        html`<button onclick=${event => _deleteFile(event)}
                     class="toolbar-button" uk-icon="trash" title="Delete"></button>`
    }
    ...
    if (createdFile.isUploading) {
      ...
      return html`
        <div data-file-id="${createdFile.id}" data-filename="${createdFile.filename}">
          <div>
      ...
      return html`
        <div data-file-id="${createdFile.id}" data-filename="${createdFile.filename}">
          <div>
            <a href="${hrefAttr}" class="uk-link-text uk-text-bold" download>
              ${truncatedFilename}
            </a>
            <button onclick=${event => _copyToClipboard(event, window.location.origin + hrefAttr)}
                    class="toolbar-button" uk-icon="icon: copy" title="Copy link"></button>
            <span class="uk-text-meta">(${createdFile.contentType})</span>
          </div>
          <div class="uk-flex uk-flex-between uk-flex-middle">
            <div class="file-meta-data uk-text-meta">
