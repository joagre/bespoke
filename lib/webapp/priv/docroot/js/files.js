...
const Files = (function () {
    ...
    _fileUpload.addEventListener("change", async function (e) {
      try {
        ...
        for (const file of allFiles) {
          ...
          // REST: Insert file into database
          ...
          const uploadController = Bespoke.upload(file, onProgress, extraHeaders);
  ...
  function _createFileTemplate(insertedFile) {
    const truncatedFilename = insertedFile.filename.substring(0, 48);
    let deleteButton = "";
    if (insertedFile.uploaderId === Bespoke.getCookieValue("userId")) {
      deleteButton =
