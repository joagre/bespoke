...
class Bespoke {
  constructor() {
    ...
    if (this.getLocalItem("readCache", null) == null) {
      this.setLocalItem("readCache", []);
    }
  ...
  gotoPage(event, destination, postId) {
    ...
    if (event != null && event.target.tagName === "IMG") {
      const lightboxDiv = event.target.closest('div[uk-lightbox]');
      if (lightboxDiv) {
        return;
      }
    }
  ...
  async captivePortalAck() {
    ...
    const response = await fetch("/api/captive_portal_ack",
  ...
  markAsRead(postId) {
    const readCache = this.getLocalItem("readCache");
    readCache.push(postId);
    console.log("Marking post as read:", postId);
    this.setLocalItem("readCache", readCache);
  }
  ...
  uploadReadCache() {
    ...
    // REST: Upload read cache
    console.log("Uploading read cache:", readCache);
    fetch("/api/upload_read_cache", {
      method: "POST",
      body: JSON.stringify(readCache),
  ...
  isMiniBrowser() {
    ...
    const miniBrowserPatterns = [
      "CaptivePortalLogin", // Android captive portal
      "CaptiveNetworkSupport", // iOS captive portal
      "NetworkAssistant", // Huawei captive portal
    ];
    return miniBrowserPatterns.some((pattern) => userAgent.includes(pattern));
  };
}
