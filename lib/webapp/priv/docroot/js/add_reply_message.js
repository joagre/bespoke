...
const AddReplyMessage = (function () {
  ...
  async function _updatePage() {
    ...
    if (!noContent) {
      ...
      parentMessageTitle = decoder.decode(new Uint8Array(buffer, 2, titleSize));
      parentMessageBody = decoder.decode(new Uint8Array(buffer, 2 + titleSize));
    } else {
      parentMessageTitle = "<No content>";
      parentMessageBody = "<No content>";
    }

    // Update parent message
    _parentTitleElement.textContent = parentMessageTitle;
    ...
    if (_selectedRecipientUsernames.length === 0) {
      _selectedRecipientUsernames = _parentMessage.recipients.map(recipient => recipient.username);
      _selectedRecipientUsernames.push(_username);
      _recipients = _parentMessage.recipients;
    } else {
...
/*
  ...
  async function _updatePage() {
    try {
      ...
      Bespoke.assert(result.length === 1, "Expected exactly one message");
      _parentMessage = result[0];
      // REST: Read top message title (if necessary)
      _topMessageTitle = _parentMessage.title;
      if (_topMessageTitle == null) {
        const lookupMessagesResponse = await fetch("/api/read_messages", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify([_parentMessage.topMessageId]),
        });
        if (!lookupMessagesResponse.ok) {
          if (lookupMessagesResponse.status === 401) {
            Bespoke.navigateTo("loader.html");
          }
          console.error(`Server error: ${response.status}`);
          return;
        }
        const lookupMessagesResult = await lookupMessagesResponse.json();
        Bespoke.assert(lookupMessagesResult.length === 1, "Expected exactly one message");
        const topMessage = lookupMessagesResult[0];
        _topMessageTitle = topMessage.title;
      }
      _populatePage();
      _formBody.focus();
    } catch (error) {
  ...
  function _populatePage() {
    _parentTitle.innerText = _topMessageTitle;
    _parentAuthor.textContent = _parentMessage.authorUsername;
    Bespoke.formatMarkdown(_parentBody, _parentMessage.body);
  }
  ...
  async function _updateServer() {
    ...
    try {
      const payload = {
        ...
        parentMessageId: _parentMessage.id,
        topMessageId: (_parentMessage.topMessageId != null) ?
          _parentMessage.topMessageId : _parentMessage.id,
        attachments
      };
      // REST: Create top message
      const response = await fetch("/api/create_message", {
      ...
      console.error("Addition of reply message failed:", error);
    }
  }
