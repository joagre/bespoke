...
const AddReplyMessage = (function () {
  ...
  async function _updatePage() {
    ...
    if (_selectedRecipientUsernames.length === 0) {
      ...
      if (!_selectedRecipientUsernames.includes(_username)) {
  ...
  async function addNow(event) {
    Bespoke.ignoreEvent(event);
    const result = await MessageLib.createMessageAndBlobs(
      _parentMessageTitle, _formBodyElement.value, _parentMessageId, _topMessageId, _recipients,
      _selectedRecipientUsernames, _uploadControllers);
    if (result.ok) {
  ...
  function toggleQuote(event) {
    ...
    if (isHidden) {
      ...
      quoteAction.setAttribute("uk-icon", "chevron-right");
    }
  }
