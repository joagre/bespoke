...
const AddReplyMessage = (function () {
  ...
  function _searchRecipients(event) {
    ...
    const payload = {
      // Note: This ensures that a message is sent to the author of the message as well
    ...
    fetch("/api/search_recipients", {
      ...
      .then(recipients => {
        if (recipients.length == _selectedRecipientUsernames.length) {
  ...
  function _checkFormCompletion() {
    // The author of the message is always a recipient of the message
  ...
  async function _updatePage() {
    ...
    const readMessageAndBodyResult = await MessageLib.readMessageAndBody(_parentMessageId, _userId);
    if (!readMessageAndBodyResult.ok) {
      return;
    }
    let parentMessage;
    let parentMessageBody;
    if (readMessageAndBodyResult.body != null) {
      ({message: parentMessage, title: _parentMessageTitle, body: parentMessageBody} =
       readMessageAndBodyResult);
    } else {
  ...
  async function _updateServer() {
    ...
    const body = new Uint8Array(2 + _parentMessageTitle.length + _formBodyElement.value.length);
    const view = new DataView(body.buffer);
    view.setUint16(0, _parentMessageTitle.length, false);
    const encoder = new TextEncoder();
    const titleBytes = encoder.encode(_parentMessageTitle);
    ...
    // REST: Create reply message
    console.log("Creating reply message...");
    const payload = {
      parentMessageId: _parentMessageId == null ? undefined : _parentMessageId,
      topMessageId: _topMessageId,
      bodyBlobs,
      attachmentBlobs
    }
    ...
    response.json().then(result => {
      console.log("Reply message has been created:", result);
