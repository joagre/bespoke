...
const AddReplyMessage = (function () {
  ...
  function _checkFormCompletion() {
    // The author of the top message is always a recipient of the message
  ...
  async function _updatePage() {
    ...
    let topMessageBody;
    if (!noContent) {
      ({title: _topMessageTitle, body: topMessageBody} =
       await TopMessages.parseMessageBlob(await bodyResponse.blob(), _userId));
    } else {
      _topMessageTitle = "<No content>";
      topMessageBody = "<No content>";
    }

    // Update top message
    _topTitleElement.textContent = _topMessageTitle;
    _topAuthorElement.textContent = _topMessage.authorUsername;
    Bespoke.formatMarkdown(_topBodyElement, topMessageBody);
  ...
  async function _updateServer() {
    ...
    const body = new Uint8Array(2 + _topMessageTitle.length + _formBodyElement.value.length);
    const view = new DataView(body.buffer);
    view.setUint16(0, _topMessageTitle.length, false);
    const encoder = new TextEncoder();
    const titleBytes = encoder.encode(_topMessageTitle);
