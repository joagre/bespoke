...
class AddTopPost {
  ...
  _load() {
    ...
    this._formAttachmentCounter = document.getElementById("form-attachment-counter");
  ...
  _checkFormCompletion() {
    this._addButton.disabled = (this._formTitle.value.trim() === "" || this._formBody.value.trim() === "");
  }

  _updatePage() {
    const username = bespoke.getCookieValue("username");
    this._titleUsername.textContent = username;
    this._formAttachmentCounter.textContent = addAttachments.numberOfUploadedFiles();
  ...
  addNow(event) {
    ...
    const updateServer = async () => {
      ...
      const attachments =
            uploadedFiles.map(file => file.absPath.replace("/tmp/", ""));
      try {
        const payload = {
