...
// Toggle this to disable signature verification
const disableSignatureVerification = true;
...
let publicKey; // Instantiated by importPublicKey()
...
async function verifySignature(code, signature) {
  ...
  const isValid = await crypto.subtle.verify("RSASSA-PKCS1-v1_5", publicKey,
                                             signature, buffer);
  return isValid;
}
...
/*
  This listener supresses *all* unhandled rejections, not just the ones for
  fetch. That means any promise that's not caught will be silently ignored,
  which can hide genuine issues. It's a blunt instrument. I'm okay with not
  seeing any unhandled errors anywhere in the service worker though. I do this
  to avoid console errors when a "/api/subscrin_on_changes" request is aborted
...
self.addEventListener("fetch", event => {
  ...
  // If signature verification is disabled OR the request goes to /api/,
  // just pass through directly to the network (no signature checks).
  if (disableSignatureVerification || reqUrl.pathname.startsWith("/api/")) {
    event.respondWith(
      fetch(event.request).catch(err => {
        console.log("Fetch interrupted:", err);
      })
    );
    return;
  }

  // Otherwise, handle signature checks for HTML/JS, etc
  event.respondWith(
    (async () => {
      // Ensure public key is loaded before handling requests
...
async function handleHTMLorJSRequest(request) {
  ...
  // If no old signature or it differs
  let netFileResp;
  try {
