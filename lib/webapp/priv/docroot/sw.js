...
self.addEventListener("install", (event) => {
  ...
  event.waitUntil(
    importPublicKey().catch((err) => {
      console.error("Service Worker: Failed to import public key:", err);
    })
  );
});
...
async function handleHTMLorJSRequest(request) {
  ...
  // 1) Fetch the new signature from server
  let newSigBase64;
  try {
    const sigResp = await fetch(sigUrl, {cache: "no-store"});
    if (!sigResp.ok) {
      console.error("SW: signature fetch failed:", sigUrl, sigResp.status);
      return fallbackOr503(request, "Signature fetch error");
    }
    newSigBase64 = await sigResp.text();
  } catch (err) {
    console.error("SW: signature network error:", err);
    return fallbackOr503(request, "Signature network error");
  }

  // 2) Compare with old signature from IndexedDB
  const oldSigBase64 = await getStoredSignature(fileUrl);
  if (oldSigBase64 && oldSigBase64 === newSigBase64) {
    // => signature unchanged
    // serve the old verified file from FILE_CACHE_NAME (if exists)
    const fileCache = await caches.open(FILE_CACHE_NAME);
    const cachedFileResp = await fileCache.match(request);
    if (cachedFileResp) {
      console.log("SW: signature unchanged, serving cached verified file:", fileUrl);
      return cachedFileResp;
    } else {
      console.warn("SW: have old signature but missing file in cache. Refetching the file...");
      // proceed to fetch & verify below
    }
  }

  // 3) If no old signature or it differs => fetch the new file, verify it
  let netFileResp;
  try {
    netFileResp = await fetch(request, { cache: "no-store" });
    if (!netFileResp.ok) {
      console.error(`SW: File fetch error ${netFileResp.status} for`, fileUrl);
      return fallbackOr503(request, "File fetch error");
    }
  } catch (err) {
    console.error("SW: Network error fetching file:", err);
  ...
  try {
    ...
    if (!valid) {
      console.error("SW: Invalid signature for", fileUrl);
      return fallbackOr503(request, "Invalid signature");
    }
  } catch (err) {
    console.error("SW: Error verifying signature for", fileUrl, err);
    return fallbackOr503(request, "Signature verify error");
  }

  // If we reach here => valid signature.
  console.log("SW: Verified new file, caching:", fileUrl);

  // 5) Store the new signature and file in our caches
  await storeSignature(fileUrl, newSigBase64);
  const verifiedResp = new Response(fileText, {
    status: 200,
    statusText: "OK",
    headers: netFileResp.headers // preserve content-type, etc.
  });
...
async function fallbackOr503(request, msg) {
  console.warn(`SW: ${msg} => fallback or 503 for`, request.url);

  const fileCache = await caches.open(FILE_CACHE_NAME);
  const oldResp = await fileCache.match(request);
  if (oldResp) {
    console.warn("SW: Serving old file from cache as fallback.");
    return oldResp;
  } else {
